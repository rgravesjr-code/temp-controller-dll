# CanTp — Distribution Package v1.3.0
...
LabVIEW `J1939Msg(V4)` cluster of an ECD database; since v1.3.0 the frames
can also travel as the flattened NI-XNET `XNET Frame CAN` cluster array.
...
| `test_cantp.exe`, `x86\test_cantp.exe`, `linux-*\test_cantp` | Release-gate test for each target (`545 passed, 0 failed`) |
...
| `tools\xnetflat.py` | The flattened `XNET Frame CAN` cluster array in Python (flatten / unflatten / to records); lists a file saved from LabVIEW |
...
| `examples\fixtures\` | Scott's two LabVIEW-flattened `XNET Frame CAN` samples (2026-09-14) used by the oracle and the unit tests |
...
| `RECORD-FORMAT.md` | Byte-level breakdown of the U8 frame array: record fields, sizes, worked examples, payload bit numbering; §6 the XNET Frame CAN cluster array |
...
3. In LabVIEW: `CanTp_Define(slot, msg.csv row, 8, sig.csv rows, nSig)` once
   ...
   `CanTp_RxFeed` per record or `CanTp_Transfer` in read mode. Or skip the
   records: `CanTp_TransferXnet(slot, 0/1, values, n, xnet, size, 0, 0, 0, &used, &nf)`
   and `Unflatten From String` (array of XNET Frame CAN) → XNET Write (Frame CAN).
...
## Design decisions (Scott, CAN parsing thread, 2026-09-13/14)

- The frames can be returned as the flattened `XNET Frame CAN` cluster
  array, which LabVIEW unflattens in one step (`CanTp_TransferXnet`);
  no frame-length array is needed on that path. The record format stays
  for non-LabVIEW callers and logging.
- Default = the TP.CM / TP.DT frames as CAN Data for a raw frame session;
  an option returns one J1939 Data frame with the whole payload for an
  XNET J1939 session. echo? is written FALSE; the write timestamp may be 0.

## Verified

| Check | Result |
|---|---|
| Unit tests, Windows x64 and x86 | 545 / 545 |
...
| XNET Frame CAN cluster array | Scott's two LabVIEW-flattened samples decode and re-flatten byte-exact (timestamp included); 3000 random instants through the timestamp conversion vs a Python big-int reference; EEC1, EC1, RC, ET1, TCFG × 10 value sets through `CanTp_TransferXnet` in both modes vs cantools and the Python mirror |
| Raspberry Pi 5 (aarch64, glibc 2.41) | 545 / 545 (v1.3.0), plus the 32-bit `armhf` build of the same sources run under the 64-bit kernel 545 / 545; v1.1.0: live BAM loop, live RTS/CTS and ISO-TP sessions between the two bench Pis (`docs\testlogs\`) |
